...
当前版本最近一次自动测试结果：全量自动测试 421/421 通过；仓库内 66 个 JavaScript 源文件 `node --check` 通过，发布包内 45 个 JavaScript 文件通过语法检查。
...
- 自动整理连续三次没做成会暂停当天，并借「提个醒」的弹窗告诉你一声。没装提个醒、或是刚好撞上它的静默时段，就安静收场，不影响做册本身。
...
- 🗓️ **记下每天的事**
  ...
  随手记一笔、补记纪念日、安排待办，也可以记录生理期。一次性的日子和每年重复的纪念日分开管理，历史待办还能继续查看和勾选。新增「生理期规律」后，记录达到一定数量时还能看到历史周期、持续天数、下次预计区间和当前第几天。
...
- 📖 **把一天收进时光册**
  ...
  自动整理没做成也不会闷着：页面上有一条状态说明，「正在等重试」「连续三次停摆了」「伙伴际遇线还没补完」都看得见，也可以随时点「现在就手动做」自己来一次；失败的任务可以选择取消重做，旧的页面和失败记录都会留着。



- 💗 **记一笔当下的心情**
  
  今日卡上可以随手标记当天的心情，原因可留白，一天里不同的情绪变化都会按时段自然收好，不打断聊天。

- 📈 **心情线**
  
  把一天或一段日子的心情变化画成一条柔和的手绘曲线：你亲笔记下的心情是饱和的实心点，小花发现的候选用浅一号的颜色轻轻落在旁边，一眼就能分清。曲线随心情高低从薄荷渐变到粉，只看画面也能感到那一整天是明亮还是低沉。只记变化，不给心情打分。
  
  可以看单日，也可以看连续 7 / 14 / 30 天——哪几天稳稳当当、哪几天起起伏伏，铺开一看就有感觉。没记录的日子不硬补点，曲线轻轻跨过去，不给空白日子硬安情绪。
  
  你的心情线旁边还能并排看伙伴的际遇线：按那天伙伴被夸、被凶、被需要这些际遇画出来，颜色按伙伴固定，线的末端标着名字，图例点一下就能收起某一位，只想看一个人时清爽很多。
...
1. 在 Hana 里打开「拾光记」。
2. 按需要打开「情境注入」；不想让背景进入对话时，可以随时关掉，也可以只让某几位伙伴不带。
...
当前版本最近一次自动测试结果：全量自动测试 353/353 通过；仓库内 66 个 JavaScript 源文件 `node --check` 通过，发布包内 45 个 JavaScript 文件通过语法检查。
