...
var PARTNER_MOOD_COLORS = ['#6fa8a0', '#b48fc4', '#d9a26a', '#8f9f7a', '#c98a9d'];

function partnerSeriesColor(index) {
  return PARTNER_MOOD_COLORS[index % PARTNER_MOOD_COLORS.length];
}
...
function moodLineLegendHtml(series) {
  ...
  (series || []).forEach(function (s) {
    ...
    html += '<span class="moodline-legend-item"><i class="moodline-legend-mark partner" style="border:1.6px solid;border-color:' + esc(s.color || '') + '"></i>' + esc(s.name || '伙伴') + '的际遇</span>';
  });
...
function partnerMoodLineEntryTitle(entry, ownerLabel) {
  ...
  var evidenceType = ({ explicit: '原话依据', context: '上下文推测' })[entry && entry.evidenceType] || '依据待确认';
  var detail = String((entry && (entry.reason || entry.note)) || '').trim();
...
function renderMoodLineDay(date, moods, partners) {
  ...
  var series = (Array.isArray(partners) ? partners : []).map(function (partner) {
  ...
  card.innerHTML = '<div class="moodline-card-head"><span class="moodline-card-title">' + esc(moodLineDateText(date)) + '</span><span class="moodline-card-caption">' + esc(caption) + '</span></div>' +
    '<div class="moodline-plot"><div class="moodline-hover-tip" id="moodline-hover-tip" role="tooltip"></div>' + renderMoodLineSvg(owner.points, rows, 'day', moodLineDateText(date), legendSeries) + '</div>' +
    '<div class="moodline-below">' + moodLineLegendHtml(legendSeries) + '<span class="moodline-note">点心情点可以回到这一天</span></div>' +
    renderMoodLineEntries(owner.model.entries, date) +
    legendSeries.map(function (s) { return renderMoodLineEntries(s.entries, date, s.name); }).join('');
}
...
function renderMoodLineRange(dates, dayMap, partnerSeries) {
  ...
  var series = (Array.isArray(partnerSeries) ? partnerSeries : []).map(function (partner) {
  ...
  var partnerButtons = legendSeries.map(function (s) {
  ...
  card.innerHTML = '<div class="moodline-card-head"><span class="moodline-card-title">' + esc(label) + '</span><span class="moodline-card-caption">' + esc(caption) + '</span></div>' +
    '<div class="moodline-plot"><div class="moodline-hover-tip" id="moodline-hover-tip" role="tooltip"></div>' + renderMoodLineSvg(points, rows, 'range', label, legendSeries) + '</div>' +
    '<div class="moodline-below">' + moodLineLegendHtml(legendSeries) + '<span class="moodline-note">空白日期不补点，曲线轻轻跨过去</span></div>' +
...
function loadMoodLine() {
  ...
  Promise.all([
    ...
    var partnerSeries = Object.keys(byAgent).sort().map(function (agentId, index) {
      return { agentId: agentId, agentName: agents[agentId] || agentId, dayMap: byAgent[agentId], color: partnerSeriesColor(index) };
    });
...
function buildPartnerSeriesFromDay(partnerRes) {
  ...
  return Object.keys(partners).sort().map(function (agentId, index) {
    return {
      agentId: agentId,
      agentName: agents[agentId] || agentId,
      entries: partners[agentId] || [],
      color: partnerSeriesColor(index),
    };
  });
}
