...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .moodline-plot {
    width: 100%; overflow: hidden; border: 1px solid var(--line); border-radius: 16px;
  ...
  .moodline-hover-tip {
    position: absolute; z-index: 3; pointer-events: none; max-width: calc(100% - 12px);
    padding: 4px 8px; border: 1px solid #efc1ce; border-radius: 10px;
    background: var(--accent-soft); color: var(--accent-deep); box-shadow: 0 4px 12px rgba(201, 111, 135, .16);
    font-size: 10px; line-height: 1.35; white-space: nowrap; opacity: 0; transform: translate(-50%, -100%);
  ...
  .moodline-legend { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; color: var(--muted); font-size: 11px; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .moodline-entry, .mood-view-link, .moodline-dot, .moodline-hover-tip { transition: none; }
...
var dayEventCache = {};
var appSettings = { showPeriod: true, injectionEnabled: true, weatherEnabled: true, moodDiscoveryMode: 'economical' }; // 全局设置缓存（启动时从后端同步）
var weatherRegions = [];
...
function moodLineLegendHtml() {
  return '<div class="moodline-legend">' +
    ...
    '<span class="moodline-legend-note">空白处不补点</span>' +
    '</div>';
}
...
function moodLineEntryTitle(entry) {
  ...
  var evidenceType = auto ? ({ explicit: '原话依据', context: '上下文推测' }[entry.evidenceType] || '依据待确认') : '';
...
function renderMoodLineSvg(points, rows, mode, title) {
...
function renderMoodLineDay(date, moods) {
  ...
  var caption = model.entries.length + ' 笔心情 · 高低只负责形成画面节奏';
  card.innerHTML = '<div class="moodline-card-head"><span class="moodline-card-title">' + esc(moodLineDateText(date)) + '</span><span class="moodline-card-caption">' + esc(caption) + '</span></div>' +
    '<div class="moodline-plot"><div class="moodline-hover-tip" id="moodline-hover-tip" role="tooltip"></div>' + renderMoodLineSvg(points, rows, 'day', moodLineDateText(date)) + '</div>' +
    '<div class="moodline-below">' + moodLineLegendHtml() + '<span class="moodline-note">点心情点可以回到这一天</span></div>' +
    renderMoodLineEntries(model.entries, date);
}

function renderMoodLineRange(dates, dayMap) {
  var card = document.getElementById('moodline-card');
  if (!card) return;
  var left = 54, width = 684;
  ...
  if (!marked.length) {
    card.innerHTML = '<div class="moodline-card-head"><span class="moodline-card-title">近 ' + moodLineDays + ' 天</span></div>' +
  ...
  var label = '近 ' + moodLineDays + ' 天 · ' + start.slice(5).replace('-', '/') + '—' + end.slice(5).replace('-', '/');
  var total = rows.reduce(function (sum, row) { return sum + row.entries.length; }, 0);
  var caption = marked.length + ' 天有记录 · 共 ' + total + ' 笔心情';
  ...
  card.innerHTML = '<div class="moodline-card-head"><span class="moodline-card-title">' + esc(label) + '</span><span class="moodline-card-caption">' + esc(caption) + '</span></div>' +
    '<div class="moodline-plot"><div class="moodline-hover-tip" id="moodline-hover-tip" role="tooltip"></div>' + renderMoodLineSvg(points, rows, 'range', label) + '</div>' +
    '<div class="moodline-below">' + moodLineLegendHtml() + '<span class="moodline-note">空白日期不补点，曲线轻轻跨过去</span></div>' +
    '<div class="moodline-entries">' + dayButtons + '</div>';
}
...
function updateMoodLineToolbarLabel() {
  ...
  if (moodLineMode === 'day') label.textContent = moodLineDateText(moodLineAnchorDate);
    ...
    var dates = moodLineRangeDates(moodLineAnchorDate, moodLineDays);
    label.textContent = '近 ' + moodLineDays + ' 天 · ' + dates[0].slice(5).replace('-', '/') + '—' + dates[dates.length - 1].slice(5).replace('-', '/');
  }
}
...
function loadMoodLine() {
  ...
  card.innerHTML = '<div class="moodline-empty">正在把这段心情铺开…</div>';
  if (moodLineMode === 'day') {
    var date = moodLineAnchorDate;
    api('api/moods/' + date).then(function (res) {
      if (requestId !== moodLineRequestSeq) return;
      if (!res || !res.ok) {
        card.innerHTML = '<div class="moodline-empty">这天的心情还没读出来，稍后再试。</div>';
        return;
      }
      renderMoodLineDay(date, res.moods || []);
    }).catch(function () {
      if (requestId === moodLineRequestSeq) card.innerHTML = '<div class="moodline-empty">这天的心情还没读出来，稍后再试。</div>';
    });
    return;
  }
  var dates = moodLineRangeDates(moodLineAnchorDate, moodLineDays);
  var maps = moodLineMonths(dates).map(function (month) {
    return api('api/moods?month=' + month).then(function (res) { return res && res.ok && res.days ? res.days : {}; }).catch(function () { return {}; });
  });
  Promise.all(maps).then(function (monthMaps) {
    if (requestId !== moodLineRequestSeq) return;
    var dayMap = {};
    monthMaps.forEach(function (map) { Object.assign(dayMap, map); });
    renderMoodLineRange(dates, dayMap);
  }).catch(function () {
...
function shiftMoodLine(direction) {
  var delta = moodLineMode === 'range' ? Number(direction || 0) * moodLineDays : Number(direction || 0);
  moodLineAnchorDate = moodLineDateShift(moodLineAnchorDate || todayMoodLineDate(), delta);
  loadMoodLine();
}
...
function queueSummaryDates(dates, messageEl, clearBatch, trigger) {
  ...
  if (messageEl) messageEl.textContent = '正在把这一页放到后台…';
  else toast('正在把这一页放到后台…');
