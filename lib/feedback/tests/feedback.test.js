...
test("feedback：默认 bus 返回纯思考时自动重试并清洗正文", async () => {
  const calls = [];
  const ctx = makeCtx({
    bus: {
      request: async (topic, payload) => {
        ...
        return calls.length === 1
          ? { choices: [{ message: { content: [{ type: "thinking", thinking: "hidden" }] } }] }
          : { text: "重试后的可见正文" };
      },
    },
  });
  const fb = new Feedback({ ctx, config: { repo: "o/r" } });
  const r = await fb.chat({ message: "设置有点问题" });
  assert.equal(r.ok, true);
  assert.equal(r.reply, "重试后的可见正文");
  assert.equal(calls.length, 2);
  assert.match(calls[1].payload.messages.at(-1).content, /直接返回最终可见正文/);
  assert.equal(calls[1].payload.reasoningLevel, "off");
});
