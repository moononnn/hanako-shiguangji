 ...
 * @param {boolean} params.hasFestivalGreeting 当天是否有配置节日问候的节日；有时每轮带入问候要求，避免第一次漏掉后后续轮次失去提示
...
export function shouldInject({
  ...
  // 有节日问候引导的节日，每轮都带提醒要求：第一次适合的闲聊不能漏；助手看到对话历史后应避免重复问候。
  if (hasFestivalGreeting) {
    return {
      should: true,
      reason: "festival-greeting",
      newState: withContext({ ...lastState, lastInjectAt: nowTs, lastDateKey: dk }),
    };
  }
...
export function buildInjectionText({
  ...
  if (festivalHint) {
    ...
    const timeBoundary = now.getHours() < 18
      ...
      : `当前本地时间是 ${localTime}，不能只凭时刻断言此刻已天黑或月亮可见；除非天气或用户明确提供依据，否则不要写成已经看到了月亮/星星或晴朗夜空，夜景用期待、祝愿或条件语气。`;
  ...
  const festivalReplyRule = festivalHint
    ? `【节日问候要求】如果${person}这轮是在闲聊、打招呼或分享日常，把节日问候当成本轮主要情感内容，不要缩成一张“节日快乐”的贴纸。回复至少有两层：先自然表达${festivalHint.name}祝福，再把本条节日氛围里的具体意象转成一句像自己在分享的感受（例如月色、桂花、茶或夜色）；不能只说“${festivalHint.name}快乐”或复述“今天是${festivalHint.name}”，也不要逐字复述引导说明。天气和普通寒暄只能点缀，不能代替节日氛围。整体像给老朋友的短短问候，有画面、有心意，不写成贺卡长文。若${person}正在问正事或请求帮助，先把正事答好，节日问候不是必需，可顺路自然带一句；不合适就不提。只有先前已经同时表达祝福和具体节日意象，才算问候过；若之前只说了节日名称或一句“${festivalHint.name}快乐”，下次合适的闲聊里补上具体节日分享，但不要重复那句祝福。`
    : "";
