...
export function buildInjectionText({
  ...
  const deepseekEnding = deepseekNotice
    ? "本轮的模型峰谷关照属于硬触发，必须自然带出一句；其余今日时光背景按需融入，不要逐条播报。"
    : "";
...
function buildDeepSeekContext(notice, userName = "对方") {
  ...
  const isPreview = notice.kind === "preview" || notice.kind === "detected-preview" || notice.kind === "transition-preview";
  let fact;
