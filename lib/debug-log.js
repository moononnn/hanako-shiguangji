...
let LOG_FILE = path.join(DATA_DIR, "debug.log");
const MAX_LOG_SIZE = 500 * 1024;

export function configureDebugLog(dataDir) {
  if (typeof dataDir !== "string" || !dataDir.trim()) return DATA_DIR;
  DATA_DIR = path.resolve(dataDir);
  LOG_FILE = path.join(DATA_DIR, "debug.log");
  return DATA_DIR;
}
