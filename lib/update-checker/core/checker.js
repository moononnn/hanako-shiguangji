...
// 降级：GitHub 不可用（限流/404/超时/断网）时返回 hasUpdate:false + 友好 message 和仓库地址，不抛错
...
function manualCheckMessage(message, repo) {
  return `${message}，请手动查看仓库：${releasesUrl(repo)}`;
}
...
export class UpdateChecker {
  ...
  async _fetchLatest(repo, current) {
    ...
    const fetcher = network && typeof network.fetch === "function"
      ...
      : null;
    if (!fetcher) {
      this.ctx?.log?.warn?.("[update-checker] 宿主网络能力不可用，拒绝直连 GitHub");
      return { ok: true, hasUpdate: false, current, latest: current, latestTitle: "", releaseUrl: releasesUrl(repo), message: manualCheckMessage("检查失败（暂时无法自动联网）", repo), manualCheck: true, _transient: true };
    }
    try {
      ...
      if (resp.status === 404) {
        ...
        return { ok: true, hasUpdate: false, current, latest: current, latestTitle: "", releaseUrl: releasesUrl(repo), message: manualCheckMessage("这个仓库还没有发布版本", repo), manualCheck: true };
      }
      if (!resp.ok) {
        this.ctx?.log?.warn?.("[update-checker] GitHub 返回非 2xx:", `HTTP ${resp.status}`, repo);
        return { ok: true, hasUpdate: false, current, latest: current, latestTitle: "", releaseUrl: releasesUrl(repo), message: manualCheckMessage(`GitHub 暂时不可用（${resp.status}）`, repo), manualCheck: true };
      }
      ...
      try {
        ...
        return { ok: true, hasUpdate: false, current, latest: current, latestTitle: "", releaseUrl: releasesUrl(repo), message: manualCheckMessage("GitHub 返回了无法解析的内容", repo), manualCheck: true };
      }
      ...
      return { ok: true, hasUpdate: false, current, latest: current, latestTitle: "", releaseUrl: releasesUrl(repo), message: manualCheckMessage("检查失败（网络不通或超时）", repo), manualCheck: true, _transient: true };
    }
  }
}
