...
test("check：404（仓库无 release）优雅降级并保留仓库入口", async () => {
  const { fetch } = makeFetcher(() => jsonResponse({ message: "Not Found" }, 404));
  const uc = new UpdateChecker({ ctx: { network: { fetch } }, manifestPath: null });
  const r = await uc.check({ repo: "o/r" });
  assert.equal(r.hasUpdate, false);
  assert.match(r.message, /还没有发布版本/);
  assert.equal(r.releaseUrl, "https://github.com/o/r/releases");
  assert.match(r.message, /https:\/\/github\.com\/o\/r\/releases/);
  assert.equal(r.manualCheck, true);
});

test("check：限流 403 优雅降级并保留仓库入口", async () => {
...
test("check：网络异常（fetch 抛错）优雅降级并显示仓库地址", async () => {
  ...
  assert.equal(r.releaseUrl, "https://github.com/o/r/releases");
  assert.match(r.message, /请手动查看仓库：https:\/\/github\.com\/o\/r\/releases/);
  assert.equal(r.manualCheck, true);
});

test("check：宿主网络能力缺失时拒绝直连且不缓存", async () => {
  ...
  assert.equal(r1._transient, true);
  assert.match(r1.message, /检查失败/);
  assert.match(r1.message, /https:\/\/github\.com\/o\/r\/releases/);
  assert.equal(r1.manualCheck, true);
  assert.equal(r1.releaseUrl, "https://github.com/o/r/releases");
  assert.equal(r2.cached, false);
});

test("check：响应 JSON 解析失败时保留手动仓库入口", async () => {
  const { fetch } = makeFetcher(() => ({ ok: true, status: 200, json: async () => { throw new Error("bad json"); } }));
  const uc = new UpdateChecker({ ctx: { network: { fetch } }, manifestPath: null });
  const r = await uc.check({ repo: "o/r" });
  assert.equal(r.hasUpdate, false);
...
test("check：请求携带超时与响应大小限制", async () => {
