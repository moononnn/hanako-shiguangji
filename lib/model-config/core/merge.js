...
// 合并规则：自定义档的 Key 留空不覆盖已存值；Hana 档只保存 provider/model，不保存地址或 Key
...
export async function mergeModelConfig(prev, next, crypto = null) {
  ...
  if (next.hanaModel || (out.hanaModel && typeof out.hanaModel === "object")) {
    // Hana 档只记用户选择；地址、接口格式和凭据每次从 Hana 运行时解析。
    // 每次合并都顺手抹掉旧版残留的 baseUrl/api/apiKey，避免切到其他档位时把旧 Key 再写回去。
    const previous = out.hanaModel || {};
    out.hanaModel = {
      providerId: next.hanaModel?.providerId || previous.providerId || "",
      modelId: next.hanaModel?.modelId || previous.modelId || "",
    };
  }
  if (next.customModel) {
...
export function validateModelConfig(config) {
  ...
  // agent 档始终交给 Hana 工具模型槽解析（槽为空时由宿主回落伙伴主对话模型）
  return { ok: true };
}
