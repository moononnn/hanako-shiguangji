// model-config · 入口
// 三档模型配置模块：agent（跟随 Hana 工具模型槽）/ hana（从 Hana 选择后用运行时凭据直连）/ custom（自定义 API）
//
...
//     mc.setHanaModelsProvider(async () => [...]);                  // 注入模型列表，Hana 档据此解析地址/格式；凭据由运行时提供
...
//   {
//     "trust": "full-access",
//     "capabilities": ["model", "network.fetch"],   // model=跟随档/读取运行时凭据；network.fetch=Hana 与 custom 档直连
