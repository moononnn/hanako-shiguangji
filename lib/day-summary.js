...
export function isSyntheticSummaryText(text) {
  ...
  return /^\[(?:hana_context|hana_reference)\]/i.test(value) || /^\[SessionFile\]/i.test(value) || /^\[来自\s*(?:Agent|助手)[^\]]*(?:非用户本人|非本人)[^\]]*\]/i.test(value) || /测试消息|验证(?:其|这条消息)?是否能显示在.+会话|收到续接摘要/.test(value);
}
...
export function formatMessageTimestamp(value) {
  const timestamp = value instanceof Date ? new Date(value) : new Date(Number.isFinite(Number(value)) ? Number(value) : value);
  if (Number.isNaN(timestamp.getTime())) return "";
  return `${dateKey(timestamp)} ${String(timestamp.getHours()).padStart(2, "0")}:${String(timestamp.getMinutes()).padStart(2, "0")}`;
}

function selectPromptLinesWithinBudget(lines, maxChars) {
  // maxChars=0 表示调用方不设上限，保持历史调用的完整文本行为。
  const budget = Math.floor(Number(maxChars));
  if (!Number.isFinite(budget) || budget <= 0 || lines.join("\n").length <= budget) return lines;
  if (lines.length === 1 || budget < 3) return [lines[0].slice(0, Math.max(0, budget))];

  // 保留首尾并在全天范围内均匀抽取，避免只截到上午或只留下最近几句。
  for (let count = lines.length - 1; count >= 2; count -= 1) {
    const selected = Array.from({ length: count }, (_, index) => (
      lines[Math.round(index * (lines.length - 1) / (count - 1))]
    ));
    if (selected.join("\n").length <= budget) return selected;
  }

  const separator = "\n";
  const available = Math.max(2, budget - separator.length);
  const headLength = Math.ceil(available / 2);
  return [
    lines[0].slice(0, headLength),
    lines[lines.length - 1].slice(0, available - headLength),
  ];
}

export function formatMessagesForPrompt(messages, { agentName = "", includeTime = false, maxChars = 0 } = {}) {
  const rows = (Array.isArray(messages) ? messages : [])
    .map((row, index) => ({ row, index }))
    .sort((a, b) => {
      const at = Number(a.row?.ts);
      const bt = Number(b.row?.ts);
  ...
  const lines = rows.map((row) => {
    const timestamp = includeTime ? formatMessageTimestamp(row?.ts) : "";
    const prefix = timestamp ? `[${timestamp}] ` : (includeTime ? "[时间不明] " : "");
    return `${prefix}${row.role === "user" ? "我" : (agentName || row.agentId)}：${row.text}`;
  });
  return selectPromptLinesWithinBudget(lines, maxChars).join("\n");
}
